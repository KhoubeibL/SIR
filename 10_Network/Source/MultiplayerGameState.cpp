...
std::tuple<char, sf::IpAddress, unsigned short> getAddressPortFromFile() {
	...
	{ // Try to open existing file (RAII block)
		...
		unsigned short port;
		char mode;
		if (inputFile >> mode && inputFile >> ipAddress && inputFile >> port)
			return std::make_tuple(mode, ipAddress, port);
	}
	...
	outputFile << 'T' << std::endl << localAddress << std::endl << port;
	return std::make_tuple('T', localAddress, port);
}
...
, mTimeSinceLastPacket(sf::seconds(0.f))
{
	...
	std::tuple<char, sf::IpAddress, unsigned short> ipPortMode = getAddressPortFromFile();

	if (isHost)
	{
		mGameServer.reset(new GameServer(sf::Vector2f(mWindow.getSize()), std::get<2>(ipPortMode)));
		std::get<1>(ipPortMode) = "127.0.0.1";
	}
	
	if (mSocket.connect(std::get<1>(ipPortMode), std::get<2>(ipPortMode), sf::seconds(5.f)) == sf::TcpSocket::Done)
		mConnected = true;
	else
